...
Every backend's output is normalized to 24 kHz mono 16-bit WAV bytes. The
audio stitching + mp3-encoding stage downstream consumes that uniform format
...
class Voice:
    ...
    ``id`` is the provider-specific identifier that callers pass back into
    ``BaseTTS.synthesize``. The other fields are presentation-only.
    """
...
class BaseTTS(ABC):
    ...
    - Accept implementation-specific extras via ``**opts`` (e.g. ``speed``
      for hosted APIs, ``ref_text`` for voice-cloning backends).
    """
