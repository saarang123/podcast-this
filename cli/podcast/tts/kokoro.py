"""Kokoro-82M backend (local; CPU-viable).

Kokoro is a compact (~82 M param) open-source TTS model with bundled preset
voices. No voice cloning — pick a voice id from ``list_voices()``.

Requires the optional ``[kokoro]`` install. API surface targets
``kokoro >= 0.7`` and is best treated as untested until exercised end-to-end
...
class KokoroTTS(BaseTTS):
    ...
    Args:
        lang_code: Kokoro language code. ``"a"`` = American English (default),
            ``"b"`` = British English. Match this to the voices you intend
            to use (voices are prefixed ``a*`` or ``b*``).
    """
